...
exports.getAppointment=async (req,res,next)=>{
  try{
    ...
    if (!appointment){
      return res.status(404).json({sucess:false,message:`No appointment with the id of ${req.params.id}`});
    }
    res.status(200).json({
...
exports.updateAppointment=async (req,res,next)=>{
  try{
    ...
    //Make sure user is the appointment owner before updating
...
exports.deleteAppointment=async (req,res,next)=>{
  try{
    ...
    //Make sure user is the appointment owner before deleting
    if (appointment.user.toString()!==req.user.id && req.user.role !== 'admin'){
